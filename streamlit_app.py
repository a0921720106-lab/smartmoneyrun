...
with tab2:
    ...
    # 輸入框提示更新，且切片上限擴充至 20 檔
    my_stocks_input = st.text_input("請輸入股票代號，用逗號或空白分隔（最多 20 檔）", value="2330, 2317", key="my_stocks_input_box")
    my_stocks = [s.strip() for s in re.split(r'[ ,]+', my_stocks_input) if len(s.strip()) == 4][:20]
